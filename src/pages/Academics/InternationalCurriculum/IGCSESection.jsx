  ...
  return (
    <section className="section-padding" style={{ backgroundColor: 'var(--color-gold-light)' }}>
      <div className="section-container grid md:grid-cols-2 gap-12">
        <div>
          <h2 className="heading-1">Why Cambridge IGCSE?</h2>
          <span className="gold-rule" />
          <ul className="mt-5 space-y-3 list-disc pl-5">
            {whyIgcse.map((point) => (
              <li key={point} className="body-m" style={{ color: 'var(--color-ink)' }}>{point}</li>
            ))}
          </ul>
        ...
        <div className="rounded-(--radius-img) p-6 md:p-8 h-fit" style={{ backgroundColor: 'var(--color-white)' }}>
          <h3 className="heading-2">Cambridge ICE</h3>
          <span className="gold-rule" />
          ...
          <ul className="mt-4 space-y-1">
            {igcseGroups.map((group) => (
              <li key={group} className="body-m" style={{ color: 'var(--color-ink)' }}>{group}</li>
            ))}
          </ul>
